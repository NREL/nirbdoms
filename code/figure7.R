# figure 7 - time course probe independent validation plot - Scale up to 10L Example
...
indval <-  readRDS("../data/processed/dataframes/laboratoryonline_probe_scaleupval.RDS") %>% filter(experiment == "BSRD 10L PS HR") 

# filter out 10L scale up experiment

speconly_e8 <- speconly  %>% filter(experiment == "BSRD 10L PS HR")

# collect predictions, grouping by constituent type 
...
preds <-speconly_e8 %>% 
  ...
  mutate(model = "Separated Sugar Model")



# collect HPLC measurements, grouping by constituent type 
...
a <- indval %>% ggplot(aes(x=time_hours, col = constituent))+geom_point( aes( y = measured,shape = "HPLC"), size =4)+
  ...
  guides(colour = guide_legend(override.aes = list(shape = 15)),
         ...
         line = "none",
         linetype = "none")+ 
  geom_point(shape =1,aes( y = measured,shape = "HPLC"), size =4,col = "black")+xlim(-5,170)+ylim(-5,170)
...
ggsave("../figures/fig7.tiff", width  = 185,height =185 ,units = "mm", compression = "lzw")
